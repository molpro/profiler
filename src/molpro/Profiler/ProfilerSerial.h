    ...
    inline bool operator()(const T &_left, const T &_right) {
      ...
      if (false) { // check the ancestors for debugging purposes only
        ...
        if (rightroot != leftroot ||
            rightqualified.substr(0, rightqualified.find(":")) == leftqualified.substr(0, leftqualified.find(":"))) {
          std::cout << "L: " << leftroot << "////" << leftqualified << std::endl;
          std::cout << "R: " << rightroot << "////" << rightqualified << std::endl;
          if (rightroot != leftroot)
            throw std::logic_error("left and right roots not the same");
          if (rightqualified.substr(0, rightqualified.find(":")) == leftqualified.substr(0, leftqualified.find(":")))
