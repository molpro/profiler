...
//  std::cout << "Profiler::stop  "<<name<<" wall="<<now.wall<<std::endl;
    ...
    if (stopPrint_ > -1) {
        ...
        diff.name = "";
        for (std::vector<resources>::const_reverse_iterator r = resourcesStack.rbegin(); r != resourcesStack.rend();
             r++)
...
std::string
ProfilerSerial::resources::str(const int width, const int verbosity, const bool cumulative, const int precision,
    ...
    if (cumulative) {
        ...
        } else if (name != "All") name.insert(0, ".");
...
std::string ProfilerSerial::str(const int verbosity, const bool cumulative, const int precision) const {
    ...
    std::priority_queue<data_t, std::deque<data_t>, compareResources<data_t> > q(localResults.begin(),
                                                                                 localResults.end());
    std::stringstream ss;
    if (!cumulative) {
    ...
    ss << "Profiler \"" << Name << "\"";
    if (cumulative) ss << " (cumulative)";
    if (activeLevel < INT_MAX) ss << " to depth " << activeLevel;
    ss << std::endl;
    for (; !q.empty(); q.pop())
    ...
    for (resultMap::iterator parent = results.begin(); parent != results.end(); ++parent) {
        ...
        *parent->second.cumulative -= *parent->second.cumulative;
        ...
        for (resultMap::iterator child = results.begin(); child != results.end(); ++child) {
            if (parent->first == child->first || (parent->first.size() <= child->first.size() && parent->first + ":" ==
                                                                                                 child->first.substr(0,
                                                                                                                     parent->first.size() +
                                                                                                                     1))) {
                *parent->second.cumulative += child->second;
                if (parent->first != child->first)
                    parent->second.cumulative->stack = std::max(parent->second.cumulative->stack,
                                                                parent->second.stack + child->second.stack);
