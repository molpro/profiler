...
macro(LibraryManager_Project)
    ...
    get_property(defined GLOBAL PROPERTY __LibraryManager_FortranCheck DEFINED)
    if (NOT defined)
        define_property(GLOBAL PROPERTY __LibraryManager_FortranCheck
                BRIEF_DOCS "Checks fortran flags"
                FULL_DOCS "Checks fortran flags")
...
macro(__LibraryManager_parseInclDir)
    foreach (x IN LISTS inclDirRec)
        if ("${x}" MATCHES "[$]<.*:.*>")
            if ("${x}" MATCHES "[$]<INSTALL_INTERFACE:.*>")
                ...
                message("non-conforming include directory =${x}")
                continue()
            endif ()
        endif ()
    ...
    list(FILTER inclDirRec EXCLUDE REGEX "^[ ]*$")
endmacro()
